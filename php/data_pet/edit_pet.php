...
$pet = $petRepo->getPetById($id);
if (!$pet) {
    set_flash_message("Data pet tidak ditemukan.", "error");
    header("Location: data_pet.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if ($petRepo->updatePet($id, $nama, $tanggal_lahir, $warna_tanda, $jenis_kelamin, $idpemilik, $idras_hewan)) {
            set_flash_message("Data pet berhasil diperbarui.", "success");
            header("Location: data_pet.php");
            exit;
        } else {
            set_flash_message("Gagal memperbarui data pet.", "error");
        }
    } else {
    ...
    // Refresh data pet untuk ditampilkan di form jika ada error
...
<body>
    ...
    <div class="container">
        <h2>Edit Data Pet: <?= htmlspecialchars($pet['nama']) ?></h2>
        <?php display_flash_message(); ?>

        <form method="post">
            <label for="nama">Nama Pet:</label>
            <input type="text" id="nama" name="nama" value="<?= htmlspecialchars($pet['nama']) ?>" required>

            <label for="tanggal_lahir">Tanggal Lahir:</label>
            <input type="date" id="tanggal_lahir" name="tanggal_lahir" value="<?= htmlspecialchars($pet['tanggal_lahir']) ?>" required>

            <label for="warna_tanda">Warna / Tanda Khas:</label>
            <input type="text" id="warna_tanda" name="warna_tanda" value="<?= htmlspecialchars($pet['warna_tanda']) ?>">
